...
//----------------------------------------------------------------------------------
    ...
    Scene_File.prototype.create = function() {
		...
		//ステータスウインドウの配置
        this._statusWindow = new Window_SavefileStatus(555, 170, 330, 450);
		...
		//ヘルプウインドウの配置
		this._helpWindow.x = 555;
		this._helpWindow.y = 115;
...
Window_SavefileList.prototype.initialize = function(x, y, width, height) {
    Window_Selectable.prototype.initialize.call(this, 170, 200, 320, 450);
    this.activate();
    this._mode = null;
};
...
//現状[ファイル名+ID、プレイ時間、地名、持ち金]、名称変更は''の中の文字で
   　
    Window_SavefileStatus.prototype.drawFileId = function(id, x, y) {
		...
		var orisaveString = $gameVariables.value(39);
		this.drawText('Location：' + orisaveString, 2, 170, 80);
		this.drawText('Gold：' + ' ', 70, 260, 80);
		this.drawText('PlayTime：' + ' ', 70, 290, 80);
    };

    Window_SavefileStatus.prototype.drawContents = function(info, x, valid) {
        this.drawText(info.playtime, 150, 290, 200);
		/*
		if (info.location) {
		...
		*/
		...
		if (valid) {
            this.drawPartyCharacters(info, 150, 370);
        }
