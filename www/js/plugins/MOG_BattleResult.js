 ...
 * 　ツノウサギにより一部改造済み
 * 　・ヒロイン衣装替えによる変数でのピクチャ変更はアクター2番にのみ有効
 * 　・経験値・ゴールド取得のSEは「Decision5_B」を直接指定しているので
 * 　　当該SEファイルの同梱は必須
 * =============================================================================
...
　　var Moghunter = Moghunter || {};
  ...
  Moghunter.bresult_variable = Number(Moghunter.parameters['Variable'] || 1);
...
BattleResult.prototype.updateExp = function() {
	 ...
	 if (this._expOld === $gameTemp._bResult[1]) {
		 this.phaseAniClear();
		 this._phase = 2;
		 AudioManager.playSe({"name":"Decision5_B","volume":90,"pitch":100,"pan":0});
	 };
};
...
BattleResult.prototype.createActorSprite = function() {
 //ここから改造
 var h_variable = Moghunter.bresult_variable;
  var i= this._actor._actorId;

	if (i == 1){
	    var j= 0;
	  }else if (i == 2){
	    var j= $gameVariables.value(h_variable);
		};

	var img = ImageManager.loadPicture("Actor_" + this._actor._actorId + j);
		//ここまで改造
...
BattleResult.prototype.createParameters = function() {
	...
	this._par.bitmap.drawText(this._actopar[1],x,y + 37 * 1,100,32);
	this._par.bitmap.drawText(this._actopar[2],x,y + 37 * 2,100,32);
	//this._par.bitmap.drawText(this._actopar[3],x,y + 37 * 3,100,32);
	//this._par.bitmap.drawText(this._actopar[4],x,y + 37 * 4,100,32);
	//this._par.bitmap.drawText(this._actopar[5],x,y + 37 * 5,100,32);
	//this._par.bitmap.drawText(this._actopar[6],x,y + 37 * 6,100,32);
	//this._par.bitmap.drawText(this._actopar[7],x,y + 37 * 7,100,32);
	//this._par.bitmap.drawText(this._actopar[8],x,y + 37 * 8,100,32);
	var x = Moghunter.bresult_par2_x;
	var y = Moghunter.bresult_par2_y;
	this._par.bitmap.drawText(this._actor._level,x,y,100,32);
	this._par.bitmap.drawText(this._actor.mhp,x,y + 37 * 1,100,32);
	this._par.bitmap.drawText(this._actor.mmp,x,y + 37 * 2,100,32);
	//this._par.bitmap.drawText(this._actor.atk,x,y + 37 * 3,100,32);
	//this._par.bitmap.drawText(this._actor.def,x,y + 37 * 4,100,32);
	//this._par.bitmap.drawText(this._actor.mat,x,y + 37 * 5,100,32);
	//this._par.bitmap.drawText(this._actor.mdf,x,y + 37 * 6,100,32);
	//this._par.bitmap.drawText(this._actor.agi,x,y + 37 * 7,100,32);
	//this._par.bitmap.drawText(this._actor.luk,x,y + 37 * 8,100,32);
