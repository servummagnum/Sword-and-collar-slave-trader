...
//=============================================================================
/*:
 * @plugindesc v1.0.0 戦闘画面にエネミーのパラメータを表示するプラグイン(改造)
 * @author フトコロ
 *
 ...
 * @default -40
 *
 * @param Display Correction_Switch
 * @desc この番号のスイッチがonのとき表示位置をY方向にずらす
 * @type number
 * @min 1
 * @max 5000
 * @default 1
 *
 * @param Display Correction_Y
 * @desc 表示位置のY座標を入力値分ずらす
 * @type number
 * @min -9999
 * @max 9999
 * @default 0
...
(function() {
    ...
    FTKR.DEP = {
        ...
        Correction_Switch    : paramParse(parameters['Display Correction_Switch']) || 0,
        Correction_Y    : paramParse(parameters['Display Correction_Y']) || 0,
    ...
    Sprite_Enemy.prototype.updateStatusSprite = function() {
      //ここから改造
    if ($gameSwitches.value(FTKR.DEP.Correction_Switch)){
      var i = FTKR.DEP.Correction_Y ;
       } else {
         var i = 0 ;
        };

        var y = FTKR.DEP.offsetY + i;
