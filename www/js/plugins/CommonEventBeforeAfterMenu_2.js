 ...
 * @default 0
 *
 * @param Common Event ID (equipment Menu)
 * @desc 装備メニューを閉じた直後に実行するコモンイベントのIDを指定してください。
 * @default 0
...
(function(){
    ...
    const _COMMON_EVENT_ID_EM = +_PARAMETERS['Common Event ID (equipment Menu)']  || 0;
    ...
    var _Scene_Equip_terminate = Scene_Equip.prototype.terminate;
    Scene_Equip.prototype.terminate = function() {
        if (_COMMON_EVENT_ID_EM && actor(2)) {
            $gameTemp.reserveCommonEvent(_COMMON_EVENT_ID_EM);
        }
