...
Window_Base.prototype.drawShopStockNumber = function(item, inventory, x, y, width)
{
    ...
    this.drawText(Params.stockCaption, x, y * 0, captionWidth);

    var numX = x + width - (valueWidth * 2 + slashWidth);
    var stock = inventory.getStock(item);
    var stockMax = inventory.getStockMax(item);
    this.resetTextColor();
    this.drawText(stock, numX, y * 0, valueWidth, 'right');
    this.drawText('/', numX + valueWidth, y * 0, slashWidth, 'right');
    this.drawText(stockMax, numX + valueWidth + slashWidth, y * 0, valueWidth, 'right');
};
