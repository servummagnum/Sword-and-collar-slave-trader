 ...
 *
 * RPGツクールMVのデフォルトでは、
 * 敵のドロップアイテムを最大3アイテムまで限られたドロップレートで設定します。
 ...
 *    Feather Cap: x%
 *   </Enemy Drops>
 *   名前付きアイテムのリストをグループ化するメモタグを書くことができます。
 *   名前付きアイテムのドロップ率が x% になります。
 *   データベースに同じ名前のアイテムが複数ある場合、
 *   アイテム、武器、防具の順にIDが最も高いアイテムが優先されます。
 *
 ...
 *   データベースのエントリに名前が一致する場合、
 *   アイテム、武器、防具の順にIDが最も高いアイテムが優先されます。
 *   メモタグの間に条件を挿入し、
 *   ドロップに対する y% の条件付きレートの増減を生成します。
 *
 * 様々な条件:
 *
 * =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
 ...
 *- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 * ドロップが計算されて行われている時、
 * プレイヤーが持っている生きているパーティーメンバーの数をチェックし、
 * それを評価チェックに対して実行します。
 *- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 ...
 * named item COUNT EVAL
 *- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 * 持っている特定のアイテム、武器、防具、(名前付きアイテム)の数をチェックします。
 * 名前付きアイテムを選択し、
 * 複数のデータベースエントリがその名前付きアイテムの名前を共有する場合、
 * アイテム、武器、鎧の順で最も高いIDが優先されます。
 *- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 * 例:   Item 1 Count > 1: +20%
 *       Weapon 2 Count === 2: +25%
 *       Armor 3 Count <= 3: -30%
 ...
 *- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 * ドロップが計算されて行われている時、
 * プレイヤーが持っている死んでいるパーティーメンバーの数をチェックし、
 * それをevalチェックに対して実行します。
 *- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 * 例:   Dead Members > 1: +20%
 *       Dead Members === 2: +25%
 *       Dead Members <= 3: -30%
 ...
 * DEATH TURN EVAL
 *- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 * 敵が死ぬまでのターン数を比較するために評価チェックを実行します。
 * この効果には Battle Engine Core が必要です。
 *- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 * 例:   Death Turn > 5: +10%
 *       Death Turn === 5: +20%
 *       Death Turn <= 4: +30%
 ...
 * ENEMY LEVEL EVAL
 *- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 * 敵のレベルを比較するために評価チェックを実行します。
 * この効果は YEP Enemy Levels プラグインを必要とします。
 *- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 * 例:   Enemy Level === 10: +30%
 *       Enemy Level <= 5: -20%
 *       Enemy Level >= 15: +10%
 ...
 * LAST STRIKE named
 *- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 * 敵への最後の攻撃がアイテムx、スキルx、名前付きアクションかをチェックします。
 * 名前付きアクションが使用され、
 * 複数のデータベースエントリがそのアクションの名前を共有する場合、
 * スキルよりも高い順にアイテムが優先されます。
 *- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 ...
 *- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 * ドロップが計算されて行われる時、
 * パーティーメンバーの数（死んでいるか生きているか）をチェックし、
 * それをevalチェックに対して実行します。
 *- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 ...
 *- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 * x をスイッチIDに置き換えます。
 * スイッチ x がオンまたはオフであれば、条件は満たされています。
 *- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 * 例:   Switch 5 ON: +10%
 *       Switch 6 OFF: -10%
 * =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
 ...
 *- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 * x を変数IDに置き換えます。
 * 変数の値を比較して条件を満たしているか確認するためにevalチェックを実行します。
 *- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 * 例:   Variable 5 > 10: +20%
 *       Variable 6 === 11: +25%
 *       Variable 7 <= 12: -30%
 ...
 *
 * enemy.timesStruckSkill(skill ID)
 * スキルIDによって参照されたスキルによって攻撃された回数の数値を返します。
 *
 * enemy.timesStruckItem(item ID)
 * アイテムIDによって参照されてたアイテムによって攻撃された回数の数値を
 * 返します。
 *
 * enemy.timesStruckSType(skill type ID)
 ...
 *
 * enemy.timesStruckState(state ID)
 * ステートIDによって参照されるステートによって攻撃された回数の数値を返します。
 *
 * enemy.timesStruckElement(element ID)
 * 属性IDによって参照される属性によって攻撃された回数に対する数値を返します。
 *
 * ============================================================================
 * Changelog
 * ============================================================================
